...
| Shai-Hulud Detector Action  | 2 → 2.1.0    | The official [v2.1.0 release](https://github.com/gensecaihq/Shai-Hulud-2.0-Detector/releases/tag/v2.1.0) reduces false positives, while its action metadata still declares the deprecated Node 20 action runtime.                                                                                                                                             | Pinned every workflow to the actual 2.1.0 release because the publisher's floating `v2` tag remains on an older commit. GitHub currently forces the upstream action onto Node 24; the runtime annotation cannot be removed until the publisher updates its action metadata and bundled artifact.                     |

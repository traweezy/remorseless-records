...
- The staging scheduler monitor runs every ten minutes. The staging operations
  ...
  after both retention jobs. Each opens or updates one GitHub issue on failure,
  closes it after recovery, and retains sanitized daily/manual/alert evidence
  for 30 days.
...
`/live` and `/ready`, both external monitors are healthy, a manual forced-alert

...
const deleteAllCategories = async (token) => {
  ...
  while (categories.length > 0) {
    ...
    categories = categories.filter(
      (c) => !leaves.some((leaf) => leaf.id === c.id)
    )
  }
}
...
const parseArtist = (productName) => {
  ...
  if (lastSeparator !== -1) {
    const suffix = name
      .slice(lastSeparator + 3)
      .trim()
      .toLowerCase()
    if (
      /^(cd|mc|lp|cassette|vinyl|2lp|3lp|7"|tape|digital|bundle|box)/.test(
        suffix
      )
    ) {
...
const loadProducts = async () => {
  const raw = await readFile(
    resolve("tmp", "remorseless_products.json"),
    "utf8"
  )
...
const resetCategories = async () => {
  ...
  products.forEach((product) => {
    ...
    extractGenres(product).forEach(({ name, isMetal }) => {
      ...
      const existing = genres.get(key) ?? {
        slug: slugify(name) || `genre-${genres.size}`,
        isMetal: false,
      }
      existing.isMetal = existing.isMetal || isMetal
  ...
  register(musicRoot)
  const genresRoot = await createCategory({
    name: "Genres",
    handle: "genres",
    parentId: musicRoot.id,
  })
  register(genresRoot)
  const artistsRoot = await createCategory({
    name: "Artists",
    handle: "artists",
    parentId: musicRoot.id,
  })
  register(artistsRoot)
  const bundlesRoot = await createCategory({
    name: "Bundles",
    handle: "bundles",
  })
  ...
  if (metalGenres.length) {
    metalParent = await createCategory({
      name: "Metal",
      handle: "metal",
      parentId: genresRoot.id,
    })
...
const run = async () => {
  ...
  await writeFile(
    outputPath,
    JSON.stringify(
      { generated_at: new Date().toISOString(), handles: map },
      null,
      2
    ),
    "utf8"

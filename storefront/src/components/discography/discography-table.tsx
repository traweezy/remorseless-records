...
const globalFilter: FilterFn<DiscographyEntry> = (
  row,
  _columnId,
  filterValue
) => {
...
const DiscographyTable = memo(
  ({ entries, className }: DiscographyTableProps) => {
    ...
    const columns = useMemo(
      () => [
        ...
        columnHelper.accessor("title", {
          header: ({ column }) => (
            <SortableHeader label="Release" column={column} />
          ),
          cell: ({ row }) => (
        ...
        columnHelper.accessor("formats", {
          ...
          filterFn: (row, id, value) => {
            ...
            return formats.some(
              (format) =>
                format.toLowerCase() === (value as string).toLowerCase()
            )
          },
        }),
        columnHelper.accessor("tags", {
          ...
          filterFn: (row, id, value) => {
            ...
            return tags.some(
              (tag) => tag.toLowerCase() === (value as string).toLowerCase()
            )
          },
        }),
        ...
        columnHelper.accessor("availability", {
          header: ({ column }) => (
            <SortableHeader label="Availability" column={column} />
          ),
          cell: ({ getValue }) => (
            <AvailabilityBadge availability={getValue()} />
          ),
          filterFn: (row, id, value) => {
            if (!value) {
              return true
    ...
    const formatDropdownOptions: [
      PillDropdownOption<string>,
      ...Array<PillDropdownOption<string>>,
    ] = [
          ...
          <div ref={parentRef} className="h-full min-h-0 overflow-auto">
            {rows.length ? (
                          ...
                          <div className="rounded-2xl border border-border/50 bg-background/85 p-4 shadow-[0_18px_35px_-28px_rgba(0,0,0,0.75)]">
                            <div className="flex items-start gap-3">
                              <div className="shrink-0">
                                {cellById.cover
                                  ? flexRender(
                                      cellById.cover.column.columnDef.cell,
                                      cellById.cover.getContext()
                                    )
                                  : null}
                              </div>
                              <div className="min-w-0 flex-1 space-y-3">
                                <div className="space-y-1">
                                  ...
                                  {cellById.title
                                    ? flexRender(
                                        cellById.title.column.columnDef.cell,
                                        cellById.title.getContext()
                                      )
                                    : null}
                                </div>
                                <div className="space-y-1">
                                  ...
                                  {cellById.artist
                                    ? flexRender(
                                        cellById.artist.column.columnDef.cell,
                                        cellById.artist.getContext()
                                      )
                                    : null}
                                </div>
                              </div>
                            ...
                            <div className="mt-4 space-y-1">
                              ...
                              {cellById.formats
                                ? flexRender(
                                    cellById.formats.column.columnDef.cell,
                                    cellById.formats.getContext()
                                  )
                                : null}
                            </div>
                            ...
                            <dl className="mt-4 grid grid-cols-2 gap-3">
                              <div className="space-y-1">
                                ...
                                <dd>
                                  {cellById.releaseDate
                                    ? flexRender(
                                        cellById.releaseDate.column.columnDef
                                          .cell,
                              ...
                              <div className="space-y-1">
                                ...
                                <dd>
                                  {cellById.tags
                                    ? flexRender(
                                        cellById.tags.column.columnDef.cell,
                                        cellById.tags.getContext()
                                      )
                                    : null}
                                </dd>
                              </div>
                              <div className="space-y-1">
                                ...
                                <dd>
                                  {cellById.catalogNumber
                                    ? flexRender(
                                        cellById.catalogNumber.column.columnDef
                                          .cell,
                              ...
                              <div className="space-y-1">
                                ...
                                <dd>
                                  {cellById.availability
                                    ? flexRender(
                                        cellById.availability.column.columnDef
                                          .cell,
              ...
              <div className="px-4 py-6 text-center text-sm text-muted-foreground">
                Nothing matches these filters yet. Clear filters or check back

  ...
  <div className="bg-background pb-16">
    <div className="mx-auto flex w-full max-w-[1440px] flex-col gap-10 px-4 py-10 sm:px-6 lg:flex-row lg:gap-12 lg:px-8">
      <aside className="hidden lg:block lg:w-72 lg:flex-shrink-0">
        <Card className="sticky top-24 max-h-[calc(100vh-7rem)] border-border/40 bg-background/95 px-6 py-8 text-sm text-muted-foreground shadow-none backdrop-blur supports-[backdrop-filter]:bg-background/70">
          Preparing filters…
        </Card>
      </aside>
      <div className="flex-1 space-y-8">
        <Card
          as="header"
          className="sticky top-16 z-10 border-border/40 bg-background/95 px-4 py-5 backdrop-blur supports-[backdrop-filter]:bg-background/70 lg:px-8"
        >
          ...
          <div className="grid gap-6 sm:grid-cols-2 xl:grid-cols-3 2xl:grid-cols-4">
            {Array.from({ length: 12 }).map((_, index) => (
              <Card
                key={index}
                variant="inset"
                className="space-y-3 border-border/40 bg-transparent p-4"
              >

...
export const reuseResolvedProductOptions = (
  ...
  const optionIds = product.options.map((option) => {
    ...
    const candidates = existingOptions
      .filter(
        (candidate) => normalizedOptionValue(candidate.title) === titleKey
      )
      .filter((candidate) => {
...
const buildImportPlan = (
  ...
  resolvedIds?: Pick<
    NormalizedCsvResult,
    "resolvedProductIds" | "resolvedVariantIds" | "resolvedProductOptions"
  >
): {
  ...
  resolvedIds?.resolvedVariantIds?.forEach(({ handle, sku, id }) => {
    if (
      ...
      sku &&
      id &&
      !variantIdByHandleSku.has(`${handle}\u0000${sku}`)
    ) {
...
export const POST = async (
  ...
  const body =
    req.validatedBody ?? (req.body as ImportProductsBody | undefined) ?? {}
  ...
  const resolvedKey =
    body.file_key ?? body.fileKey ?? body.key ?? body.filename ?? body.fileName

...
const secret = ["privacy", "backend", "unit", "test", "key"]
  .join("-")
  .repeat(2);
...
describe("POST /store/privacy-request", () => {
  it("sends a bounded request record and returns its opaque ID", async () => {
    const sendEmail = jest.fn<ReturnType<PublicFormEmailSender>, Parameters<PublicFormEmailSender>>(
      async () => undefined,
    );
  ...
  it("prevents a contact proof from being replayed for privacy", async () => {
    ...
    await createHandler(sendEmail)(requestFixture(validPayload(), "contact"), res);

...
const VariantTextField = memo<VariantTextFieldProps>(
  ({ field, hint, id, label, list, multiline = false, placeholder }) => {
    ...
    const renderControl = useCallback(
      ...
      [field.name, handleBlur, handleChange, list, multiline, placeholder, value],
    );
    return (
      <AdminFormField
    ...
    return (
      <div className="space-y-3">
        <div className="flex flex-wrap items-start justify-between gap-3">
          ...
          <Button onClick={addLine} size="small" type="button" variant="secondary">
            Add advanced field
          </Button>
        </div>
        {rows.length > 0 ? (
...
const VariantCatalogProfileWidgetContent = memo<
  VariantCatalogProfileWidgetProps
>(({ data }) => {
  ...
  const derivedLabel = deriveVariantCatalogLabel(values.format, values.formatDetail);
  ...
  const saveState: AdminSaveState = mutation.isPending || formState.isSubmitting
    ? "saving"
    : mutation.isError
      ? "error"
      : formState.isDirty
        ...
        {query.isError ? (
          ...
          <>
            <div className="grid gap-4 px-6 py-4 md:grid-cols-2 xl:grid-cols-3">
              <SummaryItem label="Native variant" value={variantNativeLabel(data)} />
              <SummaryItem label="Storefront label" value={derivedLabel} />
              <SummaryItem label="Customer state" value={customerState.label} />
              <SummaryItem label="Why customers see it" value={customerState.description} />
              <SummaryItem label="Native stock evidence" value={variantStockSummary(data)} />
              <SummaryItem label="Product release date" value={releaseDateLabel(releaseDate)} />
            </div>
            <div className="flex flex-wrap gap-2 px-6 py-4">
              <Badge color={query.data?.profile ? "blue" : "orange"}>
                {query.data?.profile ? "Catalog data saved" : "Catalog data incomplete"}
              </Badge>
              <Badge color="grey">Display label is derived</Badge>
              {values.preorderAllowed ? (
                <Badge color={releaseIsFuture ? "blue" : "orange"}>
                  {releaseIsFuture ? "Preorder eligible" : "Set a future release date"}
                </Badge>
              ) : null}
            </div>
          </>
...
export const VariantCatalogProfileWidget = memo<
  VariantCatalogProfileWidgetProps
>(({ data }) => (

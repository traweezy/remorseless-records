...
const localYearMonth = (reference: Date): { month: number; year: number } => {
...
export const easternCalendarQuarter = (
  ...
  const monthIndex = Math.floor(currentMonthIndex / 3) * 3 + offset * 3
...
export const newYorkSalesTaxQuarter = (
  ...
  const quarter = month >= 12 || month < 3 ? 3 : Math.floor((month - 3) / 3)

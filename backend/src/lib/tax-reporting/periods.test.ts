...
describe("tax reporting periods", () => {
  it("uses New York midnight across daylight-saving transitions", () => {
    expect(newYorkMidnightUtc("2026-03-08")).toBe("2026-03-08T05:00:00.000Z")
    expect(newYorkMidnightUtc("2026-11-01")).toBe("2026-11-01T04:00:00.000Z")
  ...
  it("builds the March through February sales-tax year", () => {
    expect(newYorkSalesTaxYear(new Date("2026-01-15T12:00:00.000Z"))).toEqual({
      endDate: "2026-03-01",
  ...
  it("builds current and previous calendar-month filing periods", () => {
    expect(newYorkCalendarMonth(new Date("2026-01-15T12:00:00.000Z"))).toEqual({
      endDate: "2026-02-01",
  ...
  it("builds calendar years and Pennsylvania half-years", () => {
    expect(easternCalendarYear(new Date("2026-07-15T12:00:00.000Z"))).toEqual({
      endDate: "2027-01-01",
      startDate: "2026-01-01",
    })
    expect(
  ...
  it("exposes only valid period choices for each filing state", () => {
    ...
    expect(
      taxPeriodPresetOptions("PA").map(({ value }) => value)
    ).not.toContain("current-year")
  })

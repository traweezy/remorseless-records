...
const fetchAdminKeyWithPatience = async (host, masterKey) => {
  ...
  const maxDelayMs = parseIntegerEnv(
    "MEILISEARCH_KEY_FETCH_MAX_DELAY_MS",
    15000
  )
    ...
    try {
      ...
      if (!response.ok) {
        ...
        if (!adminKeyEntry?.key) {
          console.error(
            `${attemptLabel} No admin key present in Meilisearch response.`
          )
          return null
        }
      ...
      console.warn(
        `${attemptLabel} Network error fetching Meilisearch key: ${error.message}`
      )
    }
  ...
  console.error(
    "[meilisearch][admin-key] Exhausted retry budget without success."
  )
  return null
}
...
const ensureMeilisearchAdminKey = async () => {
  ...
  if (!host || !masterKey) {
    console.warn(
      "[meilisearch][admin-key] Host or master key missing; skipping fetch."
    )
    return
  }
...
const prepareSearchIfEnabled = () => {
  ...
  try {
    ...
    runMedusaCommand([
      "exec",
      resolveBuiltScript("sync-meilisearch-settings.js"),
    ])
